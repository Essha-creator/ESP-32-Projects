...
/* Helper function to publish text */
void publishText(const char* text)
{
  client.publish("tea/status/text", text);
  Serial.println(text);
}
...
void setup()
{
  ...
  while (!client.connected())
  {
    if (client.connect("ESP32_Tea_Text"))
    {
  ...
  /* SEND TEXT MESSAGES (EXACTLY AS YOU WANT) */
  publishText("strong tea");
  delay(2000);

  publishText("classic");
  delay(2000);

  publishText("sugar-free");
  delay(2000);

  publishText("2 cups are prepared");
  delay(2000);

  publishText("5 minutes waiting");
}
